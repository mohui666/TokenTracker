...
Read [CLAUDE.md](./CLAUDE.md) only for architecture, relevant commands, and project conventions needed by the current task.

For Codex, the Astra scope and validation rules in this file govern the workflow. Generic mandatory planning, release, benchmark, or approval steps in `CLAUDE.md` do not expand the current request.
